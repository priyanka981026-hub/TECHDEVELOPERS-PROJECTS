...
if df is not None:
    ...
    if run_button:
        ...
        with col1: st.metric('Total headlines', f'{total}')
        with col2: st.metric('Positive', f'{pos} ({pos/total:.1%})')
        with col3: st.metric('Negative', f'{neg} ({neg/total:.1%})')
        ...
        try:
            ...
            c1, c2 = st.columns(2)
            with c1: st.markdown('**Top Positive**'); st.table(pos_top[[headline_col, '_clean_headline']].reset_index(drop=True).head(10))
            with c2: st.markdown('**Top Negative**'); st.table(neg_top[[headline_col, '_clean_headline']].reset_index(drop=True).head(10))
        except Exception as e:
